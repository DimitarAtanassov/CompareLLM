...
class ProviderRegistry:
    ...
    def get_chat(self, target: str) -> ChatProvider:
        ...
        if kind == "openai":
            from comparellm.providers.openai_compat import OpenAICompatChat

            return OpenAICompatChat(spec, model, timeout)
        if kind == "anthropic":
            from comparellm.providers.anthropic_provider import AnthropicChat

            return AnthropicChat(spec, model, timeout, self._settings.anthropic_default_max_tokens)
        if kind == "gemini":
            from comparellm.providers.gemini_provider import GeminiChat

            return GeminiChat(spec, model, timeout)
        if kind == "cohere":
            from comparellm.providers.cohere_provider import CohereChat
    ...
    def get_embedder(self, target: str) -> EmbeddingProvider:
        ...
        if kind == "openai":
            from comparellm.providers.openai_compat import OpenAICompatEmbeddings

            return OpenAICompatEmbeddings(spec, model, timeout)
        if kind == "gemini":
            from comparellm.providers.gemini_provider import GeminiEmbeddings

            return GeminiEmbeddings(spec, model, timeout)
        if kind == "cohere":
            from comparellm.providers.cohere_provider import CohereEmbeddings

            return CohereEmbeddings(spec, model, timeout)
        if kind == "voyage":
            from comparellm.providers.voyage_provider import VoyageEmbeddings
