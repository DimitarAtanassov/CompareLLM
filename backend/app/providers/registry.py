...
class ProviderRegistry:
    ...
    def _build_spec(self, provider: ProviderConfig) -> ProviderSpec:
        ...
        if provider.requires_api_key and not api_key:
            log.warning(
                "provider_api_key_missing", provider=provider.key, env=provider.api_key_env
            )
    ...
    def get_chat(self, target: str) -> ChatProvider:
        ...
        if kind == "anthropic":
            ...
            return AnthropicChat(
                spec, model, timeout, self._settings.anthropic_default_max_tokens
            )
        if kind == "gemini":
    ...
    def get_embedder(self, target: str) -> EmbeddingProvider:
        ...
        spec = self._spec(provider_key)
        kind = (
            "openai" if spec.type in _OPENAI_EMBED_TYPES or spec.wire == "openai" else spec.type
        )
        timeout = self._settings.request_timeout_seconds
